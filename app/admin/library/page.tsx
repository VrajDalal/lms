...
export default function Library() {
    ...
    const updateLibraryBookDetails = async (bookNo: string) => {
        ...
        if (!selectedBook?.bookNo || !selectedBook?.bookName || !selectedBook?.bookAuthorName || !selectedBook?.bookPublisherName || !selectedBook?.bookQty) {
            console.error("Invalid book details", selectedBook);
            return;
        }

        try {
            setLoading(true)
            const updateLibraryBookDetailsResponse = await fetch(`/api/admin/updateLibraryBook/${bookNo}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json'
                },
                                                ...
                                                <div className="mb-4 lg:mb-12">
                                                    <div className="flex flex-col md:flex-row lg:flex-row gap-4 justify-center lg:justify-center">
                                                        ...
                                                        <div className="flex-1 lg:flex-none lg:w-1/4">
                                                            ...
                                                            {/* Pattern Error Message */}
                                                            {bookDetails.bookQty && !/^[0-9]+$/.test(bookDetails.bookQty.toString()) && (
